...
final flatOpeningsListProvider =
    FutureProvider.autoDispose<IList<PuzzleOpeningData>>((Ref ref) async {
      final families = await ref.watch(
        puzzleOpeningsProvider(PuzzleOpeningSort.popular).future,
      );
      return families
          .map(
            (f) => [
              ...
              ...f.openings.map(
                (o) =>
                    (key: o.key, name: '${f.name}: ${o.name}', count: o.count),
              ),
            ],
          )
          .expand((e) => e)
...
final puzzleOpeningNameProvider = FutureProvider.autoDispose
    .family<String, String>((Ref ref, String key) async {
