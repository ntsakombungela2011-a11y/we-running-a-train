...
Future<Widget> _makeApp(WidgetTester tester, {FilePickerResult? pickResult}) =>
    makeTestProviderScopeApp(
      ...
      overrides: pickResult != null
          ? {
              pickPgnFileProvider: pickPgnFileProvider.overrideWithValue(
                () async => pickResult,
              ),
            }
          : null,
...
void main() {
  ...
  group('Clipboard paste', () {
    testWidgets(
      'Valid single-game PGN navigates to analysis screen immediately',
      (tester) async {
        const pgn = '[White "A"]\n[Black "B"]\n\n1. e4 e5 *';
        _mockClipboard(pgn);
    ...
    testWidgets(
      'Tapping the text field also pastes and navigates for valid PGN',
      (tester) async {
        const pgn = '1. e4 e5 *';
        _mockClipboard(pgn);
    ...
    testWidgets(
      'Multi-game PGN from clipboard navigates to the game list screen',
      (tester) async {
        const pgn =
            '[Event "Test Event"]\n'
            '[White "Player A"]\n'
    ...
    testWidgets('Multi-game clipboard: selecting a game opens analysis', (
      tester,
    ) async {
      const pgn =
          '[Event "Test Event"]\n'
    ...
    testWidgets(
      'Single-game PGN with headers navigates to analysis with correct content',
      (tester) async {
        const pgn = '''
[Event "Test Game"]
...
1. e4 e5 2. Nf3 Nc6 3. Bb5''';
  ...
  group('PGN file import', () {
    testWidgets('Single-game PGN file navigates to analysis screen', (
      tester,
    ) async {
      const pgn =
          '[Event "Test Game"]\n'
      ...
      await tester.tap(
        find.widgetWithText(FilledButton, 'Or import a PGN file'),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('Multi-game PGN file navigates to the game list screen', (
      tester,
    ) async {
      const pgn =
      ...
      expect(
        find.textContaining('BWL2025/26 Heilbronn-Deizisau II'),
        findsNWidgets(2),
      );
    ...
    testWidgets(
      'Search bar shows if there are more than 5 games and filters correctly',
      (tester) async {
        const pgn =
            '[Event "BWL2025/26 Heilbronn-Deizisau II"]\n'
            '[White "Zuferi, Enis"]\n'
