...
void main() {
  group('CorrespondenceClock', () {
    ...
    testWidgets(
      'resets timeLeft when resetId changes (active true -> false, new duration)',
      (tester) async {
        await tester.pumpWidget(
          _buildClock(
            duration: const Duration(minutes: 5),
    ...
    testWidgets(
      'resets timeLeft when resetId changes (active false -> true, new duration)',
      (tester) async {
        await tester.pumpWidget(
          _buildClock(
            duration: const Duration(minutes: 3),
    ...
    testWidgets(
      'resets timeLeft when resetId changes even if duration is unchanged',
      (tester) async {
        await tester.pumpWidget(
          _buildClock(
            duration: const Duration(minutes: 5),
    ...
    testWidgets(
      'does not reset timeLeft on widget rebuild when resetId is unchanged',
      (tester) async {
        await tester.pumpWidget(
          _buildClock(
            duration: const Duration(minutes: 5),
